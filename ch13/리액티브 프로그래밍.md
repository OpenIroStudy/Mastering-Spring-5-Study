...
```
* * Mono의 경우 다수의 item이 없기 때문에 onComplete와 onError 메소드를 제공한다. (onNext는 없음)
* * Mono를 사용하면 완료 개념만 있는 값이 없는 비동기 프로세스를 나타낼 수 있다. 하나를 만들려면 empty Mono를 사용할 수 있다.
 * Optional과 유사하고 null을 사용하지 않아야 하기 때문에 Void를 제공하여 empty 객체를 쓰게 한다.
